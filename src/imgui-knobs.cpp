...
namespace ImGuiKnobs {
    namespace detail {
        ...
        knob<DataType> knob_with_drag(const char *label, ImGuiDataType data_type, DataType *p_value, DataType v_min, DataType v_max, float _speed, const char *format, float size, ImGuiKnobFlags flags) {
            ...
            ImGui::PushID(label);
            auto width = size == 0 ? ImGui::GetTextLineHeight() * 2.0f : size * ImGui::GetIO().FontGlobalScale;
    ...
    bool SequenceToggle(const char* label, bool* v)
    {
        ...
        // I don't want to display the label, just using it for god forsaken ID generation
        const ImVec2 label_size(0,20); //  ImGui::CalcTextSize(label, NULL, true);
